...
from old.agent.actor_critic import (
    ActorCritic,
    RobotDecision,
    apply_sequential,
    compute_entropy,
...
from old.agent.hetero_graph import build_hetero_graph
...
class Policy:
    ...
    def act_verbose(
        ...
        state         : dict,
        deterministic : bool = False,
    ) -> tuple[dict[str, int | str | None], Transition, list[RobotDecision]]:
        """Como act(), mas devolve também as decisions (logits, candidates) para debug."""
        self.net.eval()
        with torch.no_grad():
            ...
            action_indices, log_probs = apply_sequential(decisions, mode=mode)
    ...
    def update(
        ...
        for _ in range(n_epochs):
            ...
            for i, t in enumerate(traj):
                ...
                _, new_log_probs = apply_sequential(
                    decisions,
                    mode           = "log_prob",
    ...
    def _compute_gae(
        ...
        # Identifica trajetórias contíguas — (start, end) inclusivos.
        ...
        for i, t in enumerate(traj):
            if t.done:
                ...
                start = i + 1
        if start < n:
            # Último episódio inacabado: trata como truncated implícito.
            boundaries.append((start, n - 1))

        for s, e in boundaries:
            last_t = traj[e]

            # Bootstrap: 0 em terminated; V(last) em truncated/inacabado.
