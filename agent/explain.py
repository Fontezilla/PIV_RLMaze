...
Portado do agente antigo; adaptado ao contrato de estado do novo env
("clock" em vez de "tick", `assigned`/`carrying`/`node` em vez de
`assigned_box_id`/`carrying_box`/`current_node`+`goal_node`)."""
...
def explain_step(
    ...
    Mostra, por robot pendente:
      - estado (nó, alvo da tarefa, o que transporta)
    ...
    n_avail = len({bid for bid, _ in state.get("available_box_targets", [])})

    lines: list[str] = []
    header = f"=== clock={clock}  pending={len(pending_ids)}  avail_boxes={n_avail}"
    ...
    for i, decision in enumerate(decisions):
        ...
        # Recupera robot_id via decision.robot_idx (índice no tensor de
        # robots) — não usa pending_ids[i] por posição, pois
        # pending_robot_indices pode ser um subconjunto filtrado.
        ...
        box_id_assigned = assigned[0] if assigned is not None else None
        ...
        lines.append(
            f"> {robot_id} @ {r.get('node') or '<transit>'}  "
            f"goal={goal or '-'}  "
            f"carry={r.get('carrying') if r.get('carrying') is not None else '-'}  "
            f"asn={box_id_assigned if box_id_assigned is not None else '-'}"
        )
...
def _format_candidate(
    ...
    """Linha-resumo de um candidato (caixa ou idle)."""
    ctype = key[0]
    if ctype == "box":
        _, box_id, target_node = key
