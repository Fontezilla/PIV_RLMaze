...
class FactoryGraph:
    ...
    def real_nodes(self) -> list[str]:
        """Nós reais do mapa (exclui sub-nós de junction — canalização
        física sem significado para decisões de alto nível, ex. o agente)."""
        return [n for n in self.graph.nodes if not self.is_subnode(n)]

    def real_edges(self) -> list[tuple[str, str]]:
        """Arestas entre nós reais, colapsando cadeias de sub-nós (sempre
        de grau 2) — a topologia lógica do mapa, sem a canalização física."""
        edges: set[tuple[str, str]] = set()
        for n in self.real_nodes():
            for neighbor in self.neighbors(n):
                prev, target = n, neighbor
                while self.is_subnode(target):
                    nxts = [x for x in self.neighbors(target) if x != prev]
                    if not nxts:
                        break
                    prev, target = target, nxts[0]
                if target != n:
                    edges.add(self.segment_id(n, target))
        return sorted(edges)
