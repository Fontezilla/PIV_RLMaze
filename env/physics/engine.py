...
class SchedulePlayer:
    ...
    def truncate(self, t: float) -> None:
        """Corta o horário (e os troços) para manter só até `t` — usado
        quando o robot é replaneado a meio de uma tarefa (ex.: atropelado
        por outro de prioridade maior). O nó em que fica é o ponto onde
        `extend()` funde a perna nova a seguir. Recalcula `_heading` a
        partir do horário cortado, para a fusão continuar consistente."""
        self.schedule = [e for e in self.schedule if e.arrival <= t + 1e-6]
        self.legs, self._heading = build_legs(self.graph, self.schedule, self.carrying_box)
