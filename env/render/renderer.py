...
def _draw_boxes(screen, graph, tf, boxes, robot_pos):
    ...
    for box in boxes:
        ...
        elif box["status"] == "PROCESSING" and box.get("process_landed"):
            px, py = tf.node(graph, box["process_landed"])
            pygame.gfxdraw.circle(screen, px, py - NODE_RADIUS - 4 - s, s + 2, (*color, 160))
...
def play(graph: FactoryGraph, players: dict, box_log: list, title: str = "Factory RL",
         speed: float = 20.0, start_time: float = 0.0) -> None:
    """Reproduz um episódio gravado: `players` (robot_id -> SchedulePlayer),
    `box_log` (lista (t, snapshot)). `speed` = ticks de sim por segundo real
    (20 = 1x, pois 1 tick = 0.05s). `start_time` salta directamente para
    esse instante em vez de começar em t=0 (útil para ir direito a um
    momento específico já conhecido)."""
    pygame.init()
